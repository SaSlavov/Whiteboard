...
import { brush, dots, spray, calligraphy, ink } from '../../Utils/Loading-utils/loading-utils';
...
const LoadBoard = (props) => {
    ...
    const handleSave = (updatedMemory) => {
        ...
        localStorage.setItem('canvas', JSON.stringify(updatedMemory))
        fetch(`${BASE_URL}/boards/${currentBoard.id}`, {
            method: 'PUT',
            headers: {
    ...
    const handleLoad = () => {
        ...
        fetch(`${BASE_URL}/boards/${selectedBoard}`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token') || ''}`,
            },
            ...
            .then(result => {
                setCurrentBoard(result);
                    ...
                    loadedCanvas = JSON.parse(result.drawing);
                
                if (!ctx.canvas) {
                    return
                }
                loadedCanvas.map(currentDrawing => {
                    if (currentDrawing[0].tool === 'Text') {
                        writeText(ctx, {
                            text: currentDrawing[0].points.text,
                            x: currentDrawing[0].points.x,
                            y: currentDrawing[0].points.y
                        },
                            {
                                fontStyle: currentDrawing[0].points.fontStyle,
                                fontSize: currentDrawing[0].points.fontSize,
                                color: currentDrawing[0].points.color,
                            })
                    }
                    if (currentDrawing[0].tool === 'Brush') {
                        brush(currentDrawing, ctx)
                    }
                    if (currentDrawing[0].tool === 'Dots') {
                        dots(currentDrawing, ctx)
                    }
                    if (currentDrawing[0].tool === 'Ink') {
                        ink(currentDrawing, ctx)
                    }
                    if (currentDrawing[0].tool === 'Calligraph') {
                        calligraphy(currentDrawing, ctx)
                    }
                    if (currentDrawing[0].tool === 'Spray') {
                        spray(currentDrawing, ctx, getRandomInt)
                    }
                    setloaded(true)
